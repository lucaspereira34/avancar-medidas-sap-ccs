...
Local $sFile_xlsx = @DesktopDir & '/Avancar-Finalizar Notas.xlsx' ; Planilha com as notas a serem avançadas e/ou finalizadas
...
Func lerExcel()
   ...
   Local $sSegunda_data = _Excel_RangeRead($oWorkbook, Default, "C3")
	  ...
	  If $sPrimeira_nota = '' and $sPrimeira_alinea = '' and $sPrimeira_data = '' Then
		 ; Mensagem de aviso
			MsgBox(0, 'Aviso', 'A planilha está vazia ou não foram inseridas informações na primeira linha da tabela.')
		 ...
		 ; Confirma se o preenchimento foi feito corretamente
			...
			; Informação não preenchida
			   If $sNota = '' or $sAlinea = '' or $sData = '' Then
				  ; Mensagem de aviso
					 MsgBox(0, 'Aviso', 'Atenção! Verifique se há alguma informação não preenchida e rode o programa novamente.')
			...
			; Alinea incorreta
			   If $sAlinea <> 'Deferido' and $sAlinea <> 'Deferido Parcial' and $sAlinea <> 'Solicitação 90 dias' and $sAlinea <>  'Solicitação 5 anos' and $sAlinea <> 'Suspensão' and $sAlinea <> 'Cortado' and $sAlinea <> 'Indeferido' and $sAlinea <> 'A' and $sAlinea <> 'B' and $sAlinea <> 'C' and $sAlinea <> 'D' and $sAlinea <> 'E' and $sAlinea <> 'F' and $sAlinea <> 'G' and $sAlinea <> 'H' and $sAlinea <> 'I' and $sAlinea <> 'J' and $sAlinea <> 'K' and $sAlinea <> 'L' and $sAlinea <> 'M' and $sAlinea <> 'N' and $sAlinea <> 'O' and $sAlinea <> 'P' and $sAlinea <> 'Q' and $sAlinea <> 'R' and $sAlinea <> 'S' and $sAlinea <> 'T' and $sAlinea <> 'U' and $sAlinea <> 'V' and $sAlinea <> 'W' Then
				  ; Mensagem de aviso
					 MsgBox(0, 'Aviso', 'Atenção! A alínea inserida é inválida. Corrija e rode o programa novamente.')
	  ...
	  ; HÁ DUAS NOTAS OU MAIS NA PLANILHA
	  Else
		 ...
		 ; Confirma se o preenhimento foi feito corretamente
			...
			For $i = 0 to UBound($aAlinea) - 1
			   If $aAlinea[$i] <> 'Deferido' and $aAlinea[$i] <> 'Deferido Parcial' and $aAlinea[$i] <> 'Solicitação 90 dias' and $aAlinea[$i] <>  'Solicitação 5 anos' and $aAlinea[$i] <> 'Suspensão' and $aAlinea[$i] <> 'Cortado' and $aAlinea[$i] <> 'Indeferido' and $aAlinea[$i] <> 'A' and $aAlinea[$i] <> 'B' and $aAlinea[$i] <> 'C' and $aAlinea[$i] <> 'D' and $aAlinea[$i] <> 'E' and $aAlinea[$i] <> 'F' and $aAlinea[$i] <> 'G' and $aAlinea[$i] <> 'H' and $aAlinea[$i] <> 'I' and $aAlinea[$i] <> 'J' and $aAlinea[$i] <> 'K' and $aAlinea[$i] <> 'L' and $aAlinea[$i] <> 'M' and $aAlinea[$i] <> 'N' and $aAlinea[$i] <> 'O' and $aAlinea[$i] <> 'P' and $aAlinea[$i] <> 'Q' and $aAlinea[$i] <> 'R' and $aAlinea[$i] <> 'S' and $aAlinea[$i] <> 'T' and $aAlinea[$i] <> 'U' and $aAlinea[$i] <> 'V' and $aAlinea[$i] <> 'W' Then
				  ; Mensagem de aviso
					 MsgBox(0, 'Aviso', 'Atenção! A alínea inserida na linha ' & $i + 2 & ' é inválida. Corrija e rode o programa novamente.')
...
Func _log()
   ...
   ; Preenche as colunas de Status, Nota e Alínea
	  For $i = 2 to UBound($aLog, $UBOUND_ROWS) - 1
		 $aLog[$i][0] = '           '									; Status
		 $aLog[$i][1] = ' ' & $aNota[$i-2] & ' '						; Nota
		 If StringLen($aAlinea[$i-2]) = 1 Then							; Alínea
...
Func avancarMedidas()
   ...
   ; Entra em cada nota e avançaa as medidas de acordo com a Alinea
	  For $i = 0 to UBound($aNota) - 1
		 ...
		 ; Confirma se a nota está na medida correta

			; Seleciona a aba 'Medidas'
			   $sap_session.findById("wnd[0]/usr/tabsTAB_GROUP_10/tabp10\TAB11").select

			; Identifica se a nota já está finalizada
			   If WinExists('Exibir nota de serviço: Recl serv.Danos Elét') Then
			...
			; Identifica se a nota foi uma improcedente finalizada ou se está em RECLIMPR
			...
			; Identifica se a nota foi uma alinea A finalizada ou se está em COMUNCLT
			   If $aAlinea[$i] = 'A' Then
			...
			; Identifica se a nota foi uma alinea B finalizada ou se está em NOTINTER
			   If $aAlinea[$i] = 'B' Then
			...
			; Identifica se a nota está em RECLPROC ou se foi uma RECLPROC avançada para PAGAUTOR
		 ...
		 ; Se a medida não estiver na correta, pula para a próxima nota
			If $bMedida_correta = False Then
			   _SAPVKeysSend("F3")
			   ExitLoop
			EndIf

		 ; Seleciona a aba 'Informações Adicionais'
		 ...
		 If $aAlinea[$i] = 'B' Then
			; Insere a 'Data de Entrega da Documentação'
			...
			; Insere a 'Hora de Entrega da Documentação'
		 ...
		 If $aAlinea[$i] = 'Solicitação 90 dias' or $aAlinea[$i] = 'Solicitação 5 anos' Then
			...
			; Insere a 'Hora de Solicitação da Documentação'
		 ...
		 ; Identificação da linha da medida
		 ...
		 If $aAlinea[$i] = 'Deferido' or $aAlinea[$i] = 'Deferido parcial' or $aAlinea[$i] = 'C' or $aAlinea[$i] = 'D' or $aAlinea[$i] = 'E' or $aAlinea[$i] = 'F' or $aAlinea[$i] = 'G' or $aAlinea[$i] = 'H' or $aAlinea[$i] = 'I' or $aAlinea[$i] = 'J' or $aAlinea[$i] = 'K' or $aAlinea[$i] = 'L' or $aAlinea[$i] = 'M' or $aAlinea[$i] = 'N' or $aAlinea[$i] = 'O' or $aAlinea[$i] = 'P' or $aAlinea[$i] = 'Q' or $aAlinea[$i] = 'R' or $aAlinea[$i] = 'S' or $aAlinea[$i] = 'T' or $aAlinea[$i] = 'U' or $aAlinea[$i] = 'V' or $aAlinea[$i] = 'W' or $aAlinea[$i] = 'Cortado' or $aAlinea[$i] = 'Suspensão' or $aAlinea[$i] = 'Indeferido' Then
			...
			; Avança de COMUNCLT para PAGAUTOR ou FINLNOT
		 ...
		 If $aAlinea[$i] = 'A' Then
			; Avança de COMUNCLT para FINLNOT
			   $sap_session.findById("wnd[0]/usr/tabsTAB_GROUP_10/tabp10\TAB11/ssubSUB_GROUP_10:SAPLIQS0:7120/tblSAPLIQS0MASSNAHMEN_VIEWER").getAbsoluteRow($iLinha).selected = true
			   $sap_session.findById("wnd[0]/usr/tabsTAB_GROUP_10/tabp10\TAB11/ssubSUB_GROUP_10:SAPLIQS0:7120/btnFC_ERLEDIGT").press
			   $sap_session.findById("wnd[0]/shellcont/shell").selectItem("0010","Column01")
			   $sap_session.findById("wnd[0]/shellcont/shell").ensureVisibleHorizontalItem("0010","Column01")
		 ...
		 If $aAlinea[$i] = 'B' Then
			; Avança de AGUARDOC e NOTINTER para PARATEND
			   $sap_session.findById("wnd[0]/usr/tabsTAB_GROUP_10/tabp10\TAB11/ssubSUB_GROUP_10:SAPLIQS0:7120/tblSAPLIQS0MASSNAHMEN_VIEWER").getAbsoluteRow($iLinha).selected = true
			...
			; Altera PARATEND para RECLIMPR e avança para COMUNCLT
	  ...
	  If $bMedida_correta = False and $bMedida_avancada = True Then
		 $aLog[$i+2][3] = ' Medida já avançada'
	  EndIf

	  If $bMedida_correta = False and $bMedida_avancada = False Then
		 $aLog[$i+2][3] = ' Medida não corresponde a Alínea inserida'
	  EndIf
   ...
   MsgBox(0, 'Aviso', 'Concluído! Confira o LOG aberto no bloco de notas.')
